...
% Include Figure 1: Schematic overview
...
% Include stakeholder groups table
...
% NIX (.nix) - HDF5 backend with Neo interface
% NWB (.nwb) - Neurodata Without Borders
% Rationale: open formats, comprehensive metadata, existing tool ecosystem


\subsubsection*{Data Stream Linking}
% stream_id column in channels.tsv
% Links channels to neurodata objects in NWB (e.g., /acquisition/ElectricalSeries)
% Links channels to data arrays in NIX following Neo structure
% Handles multiple data streams and split files
...
% New for microelectrophysiology - NOT present in iEEG-BIDS
% Table with required/optional columns
% probe_name, type, hemisphere, coordinates (AP, ML, DV), angles

\subsubsection*{ProbeInterface Integration}
% Link to ProbeInterface library for standardized probe geometries
% model column in probes.tsv with Levels and TermURL
% Example: Neuropixels 1.0, NeuroNexus A1x32-Poly3
% Enables automated electrode position lookup from community library
...
% KEY DIFFERENCE from iEEG: dual coordinate approach

\subsubsection*{Dual Coordinate Approach}
% Without space-<label>: probe-relative coordinates (electrode positions on probe geometry)
% With space-<label>: anatomical/stereotaxic coordinates
% This differs from iEEG where positions are always in anatomical space
% Rationale: microelectrode probes have known geometries but uncertain anatomical positions

\subsubsection*{Probe-Relative Coordinates}
% Default when no space entity
% x, y, z relative to probe origin (typically tip)
% Units in probes.tsv dimension_unit column
% Essential for spike sorting and probe geometry

\subsubsection*{Surgical Coordinate Conventions}
% NEW appendix in specification
% AP (Anterior-Posterior): positive = anterior
% ML (Medial-Lateral): positive = right
% DV (Dorsal-Ventral): positive = ventral
% Angles: AP_angle, ML_angle, rotation_angle
% Reference points: Bregma, Lambda, anatomical_reference_point column

\subsubsection*{Brain Atlas Integration}
% 12+ coordinate systems for cross-species support
% Rodent: AllenCCFv3, WaxholmSpace, PaxinosWatson, FranklinPaxinos, SwansonRat, WistarRatAtlas
% Primate: CHARM, D99, PaxinosMacaque, MarmosetBrainAtlas
% Generic: Stereotaxic, Pixels, individual, Other
% Each with RRID identifiers where available
...
\begin{figure}[ht]
\centering
% Compile from scripts/microephys_ecephys_schematic.tex
% \includegraphics[width=\linewidth]{figures/microephys_ecephys_schematic}
\caption{Directory structure and file organization for extracellular electrophysiology (\texttt{ecephys}) recordings.
The schematic shows the hierarchical relationship between data files, metadata sidecars, and tabular specification files.
Numbered labels indicate the correspondence between files in the directory tree and their content structure.
The three-level hierarchy (probes, electrodes, channels) captures the physical recording setup, with probe-relative electrode coordinates and optional anatomical coordinates via the \texttt{space} entity.}
\label{fig:ecephys_schematic}
\end{figure}

\begin{figure}[ht]
\centering
% Compile from scripts/microephys_icephys_schematic.tex
% \includegraphics[width=\linewidth]{figures/microephys_icephys_schematic}
\caption{Directory structure and file organization for intracellular electrophysiology (\texttt{icephys}) recordings.
This example shows a patch-clamp dataset with brain slice samples.
The \texttt{samples.tsv} file (at the dataset root) describes tissue samples, while the recording-specific files follow the same probe-electrode-channel hierarchy as extracellular recordings.
Intracellular-specific metadata includes pipette solution, recording mode (current-clamp or voltage-clamp), and sample preparation details.}
\label{fig:icephys_schematic}
\end{figure}
